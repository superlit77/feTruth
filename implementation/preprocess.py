...
def preprocess_testing_data(testing_project):
    ...
    # 打开 ./testing_data/ 文件夹下的 JSON 文件，文件名存放在 testing_project
    with open('./testing_data/' + testing_project + '.json', encoding='utf-8') as f:
        testing_data = json.load(f)['RECORDS']  # 从 JSON 文件中加载数据，提取 RECORDS 键对应的值，将其赋给 testing_data 变量

    # 调用函数 get_testing_features 处理 testing_data，返回处理后的结果，赋给 testing_features 变量
